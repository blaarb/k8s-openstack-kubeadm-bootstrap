...
resource "openstack_networking_port_v2" "worker1_port" {
  name               = "worker1_port"
  ...
  fixed_ip {
    ...
    ip_address = var.worker1_internal-ip
  }
}

output "worker1-floating-ip" {
  value = openstack_networking_floatingip_v2.worker1_fip.address
}

resource "openstack_blockstorage_volume_v3" "worker1-main-disk" {
  name                 = "worker1-main-disk"
  description          = ""
  size                 = 8
...
resource "openstack_compute_instance_v2" "worker1" {
  ...
  block_device {
    uuid                  = openstack_blockstorage_volume_v3.worker1-main-disk.id
    source_type           = "volume"
    boot_index            = 0
  ...
  network {
    port = "${openstack_networking_port_v2.worker1_port.id}"
  }
  depends_on = [openstack_compute_secgroup_v2.allow-all, openstack_blockstorage_volume_v3.worker1-main-disk]
}

resource "openstack_compute_floatingip_associate_v2" "worker1_fip_association" {
  floating_ip = openstack_networking_floatingip_v2.worker1_fip.address
  instance_id = openstack_compute_instance_v2.worker1.id
  fixed_ip    = openstack_compute_instance_v2.worker1.access_ip_v4
}